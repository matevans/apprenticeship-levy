  ...
  "Auth Action" when {
    "the user is not logged in" must {
      "must return unauthorised" in {
        ...
        when(
          mockMicroserviceAuthConnector.authorise(any(), any())(any(), any()))
    ...
    "the user is logged in" must {
      "must return an authenticated request" in {
        val mockMicroserviceAuthConnector = mock[MicroserviceAuthConnector]

        val retrievalResult: Future[Enrolments] =
          Future.successful(Enrolments(Set(Enrolment("IR-PAYE", Seq(
            EnrolmentIdentifier("TaxOfficeNumber", "123"),
            EnrolmentIdentifier("TaxOfficeReference", "ABC")), ""))))

        when(mockMicroserviceAuthConnector.authorise[Enrolments](any(), any())(any(), any()))
